...
$(document).ready(function () {
    ...
    var qwery;
    var obj;
    var parentUri = "api/document/selectAllReviewedDocumentInCurrentUserPlate";
    var table = $('#tbll2').DataTable({
        "ajax": parentUri,
        "columns": [
            {
    ...
    $('#export_tbll').on('click', function () {
        $.ajax({
            url: "api/document/selectAllReviewedDocumentInCurrentUserPlate",
            type: 'GET',
            dataType: 'json',
    ...
    $('#tbll2 tbody').on('click', 'td.details-control', function () {
        debugger;
        ...
        $.ajax({
            ...
            success: function (data1) {
                if (data1 == 1)
                    window.location.replace("./editor_review.aspx");
            }
        });
    });
